...
export async function fetchMostVisitedBlogs(limit = 2) {
  try {
    const mostVisitedBlogs = await Blogger.find()
      .sort({ visits: -1 })
      .limit(limit); // ترتيب المدونات بناءً على عدد الزيارات بترتيب تنازلي
...
export const fetchWeeklyVisit = async () => {
  try {
    ...
    // إنشاء مصفوفة الأيام الافتراضية
    const formattedData = [
      { name: "Sun", visit: 0 },
      { name: "Mon", visit: 0 },
      { name: "Tue", visit: 0 },
    ...
    // جلب بيانات الزيارات لكل الأيام في الأسبوع الحالي
    const weeklyVisits = await WeeklyVisits.find({
      week: currentWeek,
      year: currentYear,
    });
